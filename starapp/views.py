...
from .constants import (ERROR_IS_POINTS_RANGE_VALID, ERROR_NOT_ENOUGH_POINTS, ERROR_CONSTELLATION_DOES_NOT_EXIST,
                        POINT, SPECT, CONSTELLATION, OTHER_DATA, CATALOGS,
                        REDIS_SETTINGS)
...
def _result_from_stars_with_constellation_to_dict(tag):
    ...
    stars = [{key: field for key, field in zip(star_fields, row)} 
                for row in db.session.execute(text(f"""
...
def get_data_from_constellation():
        ...
        if Constellation.query.get(tag) is not None:
            create_stars_with_constellation = text(f"""
            ...
            catalogs = [{key: field for key, field in zip(('tag', 'percentage'), catalog)} 
                        for catalog in db.session.execute(catalog_search)]
            spects = [{key: field for key, field in zip(('spect', 'percentage'), spect)} 
            ...
            return jsonify({
                ...
                "catalogs": sorted(catalogs, key=lambda x:x['percentage'], reverse=True),
        ...
        if is_points_range_valid(points):
            ...
            for star in Star.query.all():
                if is_polygon_contains_point(points, star.ra, star.dec):
                    ...
                    catalogs.update([catalog_association.catalog_tag for catalog_association 
                                    in CatalogAssociation.query.filter_by(star_id=star.id)])
            ...
            return jsonify({
                "error": ERROR_IS_POINTS_RANGE_VALID
            })
...
def segment_search():
    ...
    result = search_class.segment_search(
        minimum,
        maximum
    )
...
def sort_search():
    ...
    result = search_class.sort_search(
        page=page,
