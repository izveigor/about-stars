import pandas as pd
from .constants import (CATALOGS, INT_CATALOGS, STR_CATALOGS, POINT, SPECT, OTHER_DATA, CONSTELLATION,
                        STELLAR_CLASSIFICATION, PATH_TO_HYGDATA_V3, LIST_OF_CONSTELLATIONS)
...
def get_api():
    ...
    with click.progressbar(range(len(data)), label="Download stars:") as stars_bar, Manager() as manager:
        stars = manager.list()
...
def download_stars():
    click.echo(f'Start downloading information about stars from file "{PATH_TO_HYGDATA_V3}"')
    get_api()
