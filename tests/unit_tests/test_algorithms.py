...
class TestSearch:
    ...
    def test__init__(self, mock_return_name_of_star, client):
        ...
        testing_array = search.redis_.lrange(self.hash_ + ":dist", 0, len(result))
    ...
    def test__del__(self, client):
        ...
        for dict_ in testing_data:
            data_hash = redis_.hgetall(delete_hash + ":dist:" + dict_["name"])
            assert data_hash == {}
    ...
    def test_return_name_of_star(self, client):
        ...
        for id, result in zip(
            JsonData.data_after_api["id"], JsonData.return_name_of_star
        ):
    ...
    def test_get_value(self, client):
        ...
        for dict_ in testing_data:
            data_hash = search.redis_.hset(
                self.hash_ + ":dist:" + dict_["name"], mapping=dict_
            )
            search.redis_.rpush(self.hash_ + ":dist", data_hash)

        for index, result in enumerate(testing_data, start=0):
            assert (
                get_value(search.redis_, self.hash_, TypeSearch.DISTANCE, index)
                == result
            )
        Search.clear(hash_=self.hash_)
    ...
    def test_binary_search_if_first_value_equals_second_value(self, client):
        ...
        create_stars(0)
        search = self._return_search()
        for counter, (input_, testing_result) in enumerate(
            zip(testing_data["inputs"], testing_data["result"]), start=1
        ):
            if counter == 3:
    ...
    def test_segment_search(self, client):
        ...
        for testing_data in JsonData.segment_search:
            assert (
                search.segment_search(**testing_data["input"]) == testing_data["result"]
            )
        Search.clear(hash_=self.hash_)
    ...
    def test_sort_search(self, client):
        ...
        for testing_data, testing_result in zip(
            JsonData.sort_search, JsonData.algorithms_search_sort_result
        ):
...
class TestSearchMeta:
    def test__call__(self, client):
        ...
        third_class = ExampleClass(
            hash_="first_hash", type_search=TypeSearch.APPARENT_MAGNITUDE
        )
        fourth_class = ExampleClass(
            hash_="second_hash", type_search=TypeSearch.APPARENT_MAGNITUDE
        )
    ...
    def test_clear(self, client):
        ...
        second_class = ExampleClass(
            hash_=random_hash, type_search=TypeSearch.APPARENT_MAGNITUDE
        )
        ExampleClass.clear(hash_=random_hash)
        assert first_class != ExampleClass(
            hash_=random_hash, type_search=TypeSearch.DISTANCE
        )
        assert second_class != ExampleClass(
            hash_=random_hash, type_search=TypeSearch.APPARENT_MAGNITUDE
        )
        ExampleClass.clear(hash_=random_hash)
        del ExampleClass
