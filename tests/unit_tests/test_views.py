...
from starapp.constants import (ERROR_IS_POINTS_RANGE_VALID, ERROR_NOT_ENOUGH_POINTS,
                               ERROR_CONSTELLATION_DOES_NOT_EXIST, REDIS_SETTINGS)
import json
from collections import Counter
from starapp.views import (_counter_with_percentage, _result_from_stars_with_constellation_to_dict, 
                           TypeSearch, get_hash)
...
class TestGetDataFromConstellation:
    ...
    def test_get(self, mock_result_from_stars_with_constellation_to_dict, 
                 mock_hash, mock_search, client):
        ...
        mock_result_from_stars_with_constellation_to_dict.return_value = (result_from_stars_with_constellation, 1)
        mock_hash.return_value = random_hash
        mock_search.return_value = None

        response = client.post("/search_constellation", data=json.dumps(constellation), content_type="application/json")
    
        mock_search.assert_has_calls(
            [call(random_hash,
            type_search,
            stars=result_from_stars_with_constellation) for type_search in TypeSearch]
        )
    ...
    @patch('starapp.views.get_hash')
    @patch('starapp.views._result_from_stars_with_constellation_to_dict')
    def test_clear_session(self, mock_result_from_stars_with_constellation_to_dict, 
                 mock_hash, mock_search, mock_search_clear, client):
        create_data_for_test()
        constellation = JsonData.constellation
        ...
        client.post("/search_constellation", data=json.dumps(constellation), content_type="application/json")
    ...
    def test_result_from_stars_with_constellation_to_dict(self, client):
        ...
        """))
        db.session.commit()
        assert _result_from_stars_with_constellation_to_dict(constellation['tag']) == ([JsonData.star], 1)
        db.session.execute(text(f"""
    ...
    def test_with_not_existing_constellation(self, client):
        create_data_for_test()
        response = client.post("/search_constellation", data=json.dumps({"tag": "It's not exist!"}), content_type="application/json")
        assert f'{{"error":"{ERROR_CONSTELLATION_DOES_NOT_EXIST}"}}\n' == response.data.decode("utf-8")
    ...
    def test_get_hash(self, mock_random_randint, client):
        ...
        mock_random_randint.side_effect = [
            10, 0, 14, 7, 15, 0, 2, 2, 0, 
            6, 14, 3, 10, 9, 10, 11, 5, 0, 
            14, 14, 8, 9, 9, 4, 1, 5, 7, 11, 
            6, 4, 8, 15, 13, 3, 5, 7, 6, 13, 
            8, 13, 12, 12, 10, 3, 13, 10, 11, 
            8, 11, 7, 4, 13, 6, 8, 14, 8, 8, 
            14, 1, 13, 7, 1, 2, 0
        ]
...
class TestGetDataWithPoints:
    ...
    def test_get(self, mock_get_hash, mock_search, client):
        ...
        response = client.post("/search_points", data=json.dumps(testing_data['points']), content_type="application/json")

        mock_search.assert_has_calls(
            [call(random_hash,
            type_search,
            stars=testing_data['stars']) for type_search in TypeSearch]
        )
    ...
    def test_clear_session(self, mock_get_hash, mock_search, mock_search_clear, client):
        ...
        client.post("/search_points", data=json.dumps(testing_data['points']), content_type="application/json")
    ...
    def test_points_must_be_list(self, client):
        response = client.post("/search_points", data=json.dumps(43), content_type="application/json")
        assert response.status_code == 400
    
    def test_points_must_contain_dictionaries(self, client):
        response = client.post("/search_points", data=json.dumps([15]), content_type="application/json")
        assert response.status_code == 400

    def test_points_must_contain_ra_and_dec_attributes(self, client):
        response = client.post("/search_points", data=json.dumps([{"ra": 15}]), content_type="application/json")
        assert response.status_code == 400

    def test_is_points_range_valid(self, client):
        response = client.post("/search_points", data=json.dumps([
...
class TestSegmentSearch:
    ...
    def test_segment_search(self, mock__init__, mock_segment_search, client):
        ...
        mock_segment_search.side_effect = [search_segment["result"] for search_segment in testing_data]
        with client.session_transaction() as session:
            session['request'] = 'random_hash'
        
        for data, result in zip(testing_data, JsonData.views_search_segment_result):
            response = client.post("/segment_search", data=json.dumps({
                "type": "dist",
                **data["input"]
            }), content_type="application/json") 
...
class TestSortSearch:
    ...
    def test_sort(self, mock__init__, mock_sort_search, client):
        ...
        for testing_data, testing_result in zip(JsonData.sort_search, JsonData.views_search_sort_result):
            response = client.post("/sort_search", data=json.dumps(
                testing_data["input"]
            ), content_type="application/json")
