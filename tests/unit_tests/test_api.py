...
from tests.helpers import JsonData, check_model_fields, create_constellation_for_test, create_star_for_test, create_catalogs_for_test
from starapp.api import *
from models import Catalog, Star, CatalogAssociation, Constellation, db
from starapp.constants import (CATALOGS, INT_CATALOGS, POINT, STR_CATALOGS, SPECT, OTHER_DATA, 
                               PATH_TO_HYGDATA_V3, CONSTELLATION, LIST_OF_CONSTELLATIONS)
import numpy as np
...
class TestAddStar:
    def get_data_about_star_from_hygdata(self, hygdata, index, need_constellation=True):
        ...
        data_about_star[CONSTELLATION] = hygdata[CONSTELLATION][index] if need_constellation else None
    ...
    def test_download_stars(self, mock_get_api, runner):
        ...
        assert f'Start downloading information about stars from file "{PATH_TO_HYGDATA_V3}"' in result.output
        assert 'Information about stars have successfully downloaded!\n' in result.output
    ...
    def test_get_api(self, mock_read_csv, mock_create_catalogs, mock_create_constellations, 
                     mock_add_star, mock_progressbar, mock_click_echo,
                     mock_pool, mock_manager, client):
        ...
        for tag in CATALOGS:
            catalog_associations = [CatalogAssociation(
                **{
                    "star_id": int(testing_data_for_catalogs["id"][index]),
                    "identifier": str(testing_data_for_catalogs[tag][index]),
                    "catalog_tag": tag,
                }) for index in range(len(testing_data)) if testing_data_for_catalogs[tag][index] is not None]
        ...
        mock_manager.return_value.__enter__.return_value.list.side_effect = [[
            Star(**self.get_data_about_star_from_hygdata(JsonData.data_after_api, index, False)) for index in range(len(testing_data))
        ], testing_data_catalogs]
        ...
        for index in range(len(mock_add_star.call_args_list)):
            for key in data_from_hygdata.keys():
                assert mock_add_star.call_args_list[index][0][2][key] == data_from_hygdata[key][index]
        
        for catalog_model in Catalog.query.all():
            for index, testing_data_example in enumerate(catalog_model.catalog_associations, start=0):
                ...
                for key in catalog_association_dict.keys():
                    assert getattr(testing_data_example, key) == getattr(testing_data_catalog_associations[catalog_model.tag][index], key)
    ...
    def test_create_catalogs(self, mock_click_echo, client):
        ...
        mock_click_echo.assert_has_calls([
            ...
            *[call(f'Added catalog with tag: "{tag}"')
            for tag in CATALOGS],
    ...
    def test_add_star(self, mock_progressbar, mock_click_echo, client):
        ...
        mock_progressbar.return_value.__enter__.return_value = [con + '.txt' for con in list(testing_data[CONSTELLATION])]
    ...
    @patch('starapp.api.click.echo')
    def test_add_constellation(self, mock_click_echo, mock_create_views_for_constellation, client):
    ...
    def test_create_views_for_constellation(self, client):
        ...
        star = [{key: field for key, field in zip(star_fields, row)} for row in db.session.execute(text(
            f"""SELECT {star_fields_str} FROM stars_with_{constellation};"""
        ))][0]
        check_model_fields(Star.query.all()[0], star)
        db.session.execute(text(
            f"""DROP VIEW stars_with_{constellation};"""
        ))
        db.session.commit()