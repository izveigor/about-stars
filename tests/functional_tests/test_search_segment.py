    ...
    def test_search(self, browser: webdriver.Firefox) -> None:
        ...
        for counter, (name, inputs) in enumerate(testing_data.items(), start=1):
            ...
            for input_ in inputs:
                ...
                time.sleep(1)
                assert (
                    int(get_element_by_id("segment_number_of_stars", browser).text)
                    == input_["result"]["sum"]
                )
                assert (
                    get_element_by_id("minimum", browser).text
                    == input_["result"]["minimum"]
                )
                assert (
                    get_element_by_id("maximum", browser).text
                    == input_["result"]["maximum"]
                )
