from tests.helpers import LIVE_SERVER_URL, get_element_by_id, create_data_for_test, input_points, JsonData
from starapp.constants import ERROR_IS_POINTS_RANGE_VALID, ERROR_CONSTELLATION_DOES_NOT_EXIST, ERROR_NOT_ENOUGH_POINTS
...
def check_fields_search_statistics(testing_data, browser):
    for type_of_data in ("catalogs", "constellations", "spects"):
        ...
        for row, testing_row in zip(table.find_elements(By.TAG_NAME, 'tr'), testing_data[type_of_data]):
            first_field, percentage = [td.text for td in row.find_elements(By.TAG_NAME, 'td')]
...
class TestInputs:
    ...
    def test_change(self, browser):
        ...
        assert get_element_by_id("search_title", browser).text == CONSTELLATION_SEARCH_TITLE
        assert get_element_by_id("search_description", browser).text == CONSTELLATION_SEARCH_DESCRIPTION
        ...
        assert get_element_by_id("search_description", browser).text == POINTS_SEARCH_DESCRIPTION
    ...
    def test_search_constellation(self, browser):
        ...
        assert get_element_by_id("search_statistics", browser).value_of_css_property("display") == "flex"
        assert get_element_by_id("search_actions", browser).value_of_css_property("display") == "flex"
    ...
    def test_input_points(self, browser):
        ...
        assert get_element_by_id("clear_points", browser).value_of_css_property("display") == "none"
        assert get_element_by_id("search_points_button", browser).value_of_css_property("display") == "none"
    ...
    def test_with_not_existing_constellation(self, browser):
        ...
        assert get_element_by_id("error", browser).text == ERROR_CONSTELLATION_DOES_NOT_EXIST
